...
async function getTonalityText() {
    if (allText.value) {
...
<template>
    <div v-if="!isLoading">
        <div class="text-center mx-auto container">
            <div class="flex items-center justify-between max-sm:flex-col max-sm:gap-5 mb-5">
                <div class="flex flex-col text-left font-medium max-md:block max-md:text-left gap-3">
                    <h1 class="text-3xl max-md:text-center">Проверка тональности текста</h1>
                    <div class="flex items-center max-sm:justify-center max-sm:mt-3 gap-3">
                        <h1 @click="getExampleText(1)" class="hover:text-hover-color cursor-pointer duration-300">Пример 1
                        </h1>
                        <h1 @click="getExampleText(2)" class="hover:text-hover-color cursor-pointer duration-300">Пример 2
                        </h1>
                        <h1 @click="getExampleText(3)" class="hover:text-hover-color cursor-pointer duration-300">Пример 3
                        </h1>
                    </div>
                </div>
            ...
            <UIMyButton @click="getTonalityText">
                Проверить
            </UIMyButton>
        </div>
